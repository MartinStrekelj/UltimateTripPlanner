...
function initMap() {
  ...
  document
    .getElementById("btn-submitLocation")
  ...
  service.getDistanceMatrix(
    ...
    function (response, status) {
      if (status == "OK") {
        ...
        document.getElementById("distance-label").innerText =
          "Gas (distance = " + distance + ")";
      } else {
...
calculateBudget = () => {
  ...
  if (budget != "") {
    ...
    const gas_price = convertToNumber(
      document.getElementById("gas-price").value
    );
    const car_con = convertToNumber(
      document.getElementById("car-consumption").value
    );
    ...
    let outputMSG;
    if (Calculated_Gas != 0) {
      outputMSG = `Total budget ${budget}€ - gas costs (${distance} * 2 = ${
        distance * 2
      }km) ${Calculated_Gas}€ - other expenses (${living}€ + ${other}€) = ${Calculated_Budget}€`;
    } else {
